# Definitional proc to organize widgets for parameters.
proc init_gui { IPINST } {
  ipgui::add_param $IPINST -name "Component_Name"
  #Adding Page
  set Page_0 [ipgui::add_page $IPINST -name "Page 0"]
  ipgui::add_param $IPINST -name "COUNTWIDTH" -parent ${Page_0}
  ipgui::add_param $IPINST -name "NBITSSHIFT" -parent ${Page_0}


}

proc update_PARAM_VALUE.COUNTWIDTH { PARAM_VALUE.COUNTWIDTH } {
	# Procedure called to update COUNTWIDTH when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.COUNTWIDTH { PARAM_VALUE.COUNTWIDTH } {
	# Procedure called to validate COUNTWIDTH
	return true
}

proc update_PARAM_VALUE.NBITSSHIFT { PARAM_VALUE.NBITSSHIFT } {
	# Procedure called to update NBITSSHIFT when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.NBITSSHIFT { PARAM_VALUE.NBITSSHIFT } {
	# Procedure called to validate NBITSSHIFT
	return true
}


proc update_MODELPARAM_VALUE.COUNTWIDTH { MODELPARAM_VALUE.COUNTWIDTH PARAM_VALUE.COUNTWIDTH } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.COUNTWIDTH}] ${MODELPARAM_VALUE.COUNTWIDTH}
}

proc update_MODELPARAM_VALUE.NBITSSHIFT { MODELPARAM_VALUE.NBITSSHIFT PARAM_VALUE.NBITSSHIFT } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.NBITSSHIFT}] ${MODELPARAM_VALUE.NBITSSHIFT}
}

